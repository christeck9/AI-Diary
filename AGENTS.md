...
### 11. Arquitectura de Modales y Overlayers (Fabric Android Rendering Bug Fix)
*Fecha de Registro: 2026-07-01*

* Regla: Cada vez que se lea Rules.md correr el siguiente programa python (para auditar rapidamente estos problemas enundiados abajo): C:\AI-Diary\scripts\audit_modals.py

**Contexto y Problema Histórico:**
Bajo la nueva arquitectura de React Native (Fabric) y en modo sin puente (Bridgeless), la clase nativa de Android `Dialog` que soporta el componente `<Modal>` experimenta un defecto crítico de sincronización de medidas con el ShadowTree de Yoga. Si el modal se monta inicialmente con `visible={true}`, el evento nativo `onShow` no se dispara de forma fiable en el ciclo de vida y el modal colapsa a tamaño cero (quedando completamente invisible, transparente e inaccesible).

**Estrategia de Solución Obligatoria (Patrón de Doble Renderizado por JS):**
Para todos los modales de pantalla completa (como `BookReaderModal`, `ProfileModal`, `IntroModal`, `VoiceSettingsModal`, `VaultExplorerModal`, `TestsMenuModal`, `PsyTestModal`, etc.), se debe implementar estrictamente el siguiente patrón:

1. **Uso de Modales Transparentes y Estiramiento de Lienzo:**
   El `<Modal>` nativo debe configurarse siempre como `transparent={true}` y `statusBarTranslucent={true}`. Las dimensiones del lienzo deben forzarse de manera explícita usando el tamaño absoluto de hardware mediante `Dimensions.get('screen')`:
   ```tsx
   const { width: absoluteScreenWidth, height: absoluteScreenHeight } = Dimensions.get('screen');
   ```
2. **Forzar Redibujado usando `useEffect` (No usar `onShow` del Modal):**
   Debido a que `onShow` no es confiable, se debe usar un hook `useEffect` controlado desde JS para cambiar el estado 50ms después de que `visible` sea verdadero. Esto forzará un pase de redibujado de Yoga una vez que la ventana nativa del modal se haya presentado:
   ```tsx
   ...
   ```
3. **Inyección de Elemento de Redibujado en la Vista Contenedora:**
   En el JSX, cambia ligeramente las dimensiones de un elemento interno en base al `layoutTicket` (ej. un `View` dummy con una altura microscópica de `0.5` píxeles) para obligar a Yoga a recalcular la jerarquía completa:
   ```tsx
