...
import { InferenceEngine, ManifestoCard } from '../../components/SanctuaryUI';
...
export default function AdvancedScreen() {
  ...
  const [preferredModel, setPreferredModel] = useState('gemma3-4b-q4');
  ...
  const handleSetPreferredModel = async (modelId: string) => {
    ...
    await saveSettings({ preferredModel: modelId });
      ...
      <SafeAreaView style={[styles.container, { backgroundColor: activeTheme === 'matrix' ? '#000000' : colors.background }]}>
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
          <ScrollView contentContainerStyle={styles.scrollContent}>
            ...
            <View style={styles.section}>
              <Text style={[styles.sectionTitle, { color: colors.textSecondary }]}>{lang === 'es' ? 'PRIVACIDAD DE DATOS Y EXPORTACIONES' : 'DATA PRIVACY & EXPORT'}</Text>
              <TouchableOpacity style={styles.vaultButtonBlue} onPress={handleExportClinical}><Text style={styles.vaultButtonText}>{lang === 'es' ? 'EXPORTAR HISTORIAL DEL DIARIO (PDF)' : 'EXPORT DIARY HISTORY (PDF)'}</Text></TouchableOpacity>
              <TouchableOpacity style={styles.vaultButtonGreen} onPress={handleExportPsycho}><Text style={styles.vaultButtonText}>{lang === 'es' ? 'EXPORTAR REPORTE DE PERSONALIDAD (PDF)' : 'EXPORT PERSONALITY REPORT (PDF)'}</Text></TouchableOpacity>
            ...
            <View style={{ alignItems: 'center', marginBottom: 25, marginTop: 10, paddingHorizontal: 20 }}>
              ...
              <Text style={{ color: '#5a5a5a', fontSize: 9, letterSpacing: 2, marginTop: 5, opacity: 0.6, textAlign: 'center' }}>AI DIARY v1.9.5. AI DIARY COGNITION INTERFACE</Text>
            </View>
...
const styles = StyleSheet.create({
  ...
  vaultButtonBlue: { backgroundColor: '#5c84a8', paddingVertical: 18, borderRadius: 24, marginBottom: 15, alignItems: 'center' },
  vaultButtonGreen: { backgroundColor: '#7da885', paddingVertical: 18, borderRadius: 24, marginBottom: 15, alignItems: 'center' },
